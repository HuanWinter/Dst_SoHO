...
def AdjustWithConstraint_AH(n, t, r, A, L, P, B, W,
                            P_in, G):
    """
    n: 观测值个数
    t: 参数个数；
    r: 条件个数；
    A: 误差方程系数矩阵，数组长度为n*t，已知；
    L: 观测值向量，数组长度为n，已知；
    B: 条件方程系数阵，数组长度为r*t，已知；
    W: 条件方程自由项向量，数组长度为r，已知；
    P: 观测值权数组，当观测值独立时P为权矩阵的对角线元素，数组长度为n；
    X: 参数平差值向量，数组长度为t，待计算
    Qx: 参数平差值的权逆阵，待计算；
    V: 观测残差向量，数组长度为n，待计算；
    函数返回值—若计算成功返回单位权中误差μ；
    """
    N = np.dot(np.dot(A.T,P),A) + np.dot(np.dot(G.T,P_in),G)
    U = np.dot(np.dot(A.T,P),L)
    Y = np.dot(np.linalg.inv(N),U)
    M = np.dot(np.dot(B,np.linalg.inv(N)),B.T)
    
    K = -np.dot(np.linalg.inv(M),(np.dot(B,Y)+W))
    X = Y #+ np.dot(np.dot(np.linalg.inv(N),B.T),K)
    # print()
    V = np.dot(A,X) - L
    # Qk = linalg.inv(M)
    Qx = np.linalg.inv(N) - np.dot(np.dot(np.dot(np.linalg.inv(N),B.T), np.linalg.inv(M)),
                                   np.dot(B, np.linalg.inv(N)))
    μ = np.sqrt(np.dot(np.dot(V.T,P), V)/(n-t+r))
    
    return(X,V,Qx,μ)


def AdjustWithConstraint(n,t,r,A,L,P,B,W):
    """
    n: 观测值个数
    t: 参数个数；
    r: 条件个数；
